...
지정한 앱에 들어가면 그 언어로 맞춰집니다. **지정하지 않은 앱은 지금까지와 똑같이**
동작합니다 — 모든 앱을 자동으로 기억하면 앱을 바꿀 때마다 언어가 바뀌어 예측이
어려워지므로, 명시적으로 지정한 앱만 다르게 동작합니다.

앱 안에서 수동으로 전환하면 그게 이기고, 다시 들어올 때 지정값으로 돌아옵니다.

> 이 입력기의 한/영은 입력 소스를 바꾸는 게 아니라 내부 플래그(`self_eng_mode`)를
> 뒤집는 방식입니다. macOS 입장에선 어느 앱에서든 "NavilIME 선택됨"으로 똑같이
> 보이므로, OS의 "문서의 입력 소스로 자동 전환" 기능이 이 상태를 기억해줄 수
> 없습니다. 내부 상태는 입력기만 아니까 입력기가 직접 기억합니다.
