# NavilIME for Mac (개인 포크)

이 저장소는 **Manwoo Yi**([@navilera](https://github.com/navilera))님이 만든 [NavilIME for Mac](https://github.com/navilera/NavilIMEforMac)을 개인 용도로 포크한 것입니다.

나빌 입력기는 원작자가 세벌식 318Na 자판을 직접 디자인하고, 리눅스/윈도우/맥에서 쓸 수 있도록 입력기를 직접 구현한 프로젝트입니다. 나는 세벌식을 쓰지 못하지만 macOS 기본 한글 입력기의 불안정함이 싫어서, 이 가볍고 깔끔한 입력기를 두벌식 전용으로 커스터마이징해서 쓰고 있습니다.

## 원본 대비 변경 사항

### 세벌식 제거, 두벌식 전용
- 세벌식 318, 390 키보드 코드 전부 삭제
- 트레이 메뉴에서 키보드 선택 제거
...
### 암호 프롬프트에서 입력기 자동 비켜서기

한글 상태로 비밀번호를 치면 한글이 들어가 매번 수동으로 영문 전환해야 하던 문제를 없앴습니다.

- macOS가 secure input을 켜는 자리(잠금 화면, 키체인, 앱의 암호 필드)에서는 NavilIME가
  **스스로 조합을 멈추고 키를 그대로 흘려보냅니다.** 한/영 상태는 건드리지 않으므로
  프롬프트를 빠져나오면 원래 한글 상태 그대로입니다.
...
### 한/영 전환은 macOS 입력 소스로 일원화

입력기 내부의 한/영 모드(`self_eng_mode`)와 전용 단축키(오른쪽 Command 등)를 제거했습니다.
한/영은 **macOS 입력 소스 선택**으로만 바뀝니다 — 한글은 NavilIME, 영문은 ABC입니다.
⌃Space(기본) 또는 본인이 지정한 단축키를 쓰세요.

내부 모드는 macOS가 알 수 없는 상태였습니다. 어느 앱에서든 "NavilIME 선택됨"으로만
보이니 OS가 앱별 기억도, 암호 필드 대응도 해줄 수 없었습니다. 입력 소스로 일원화하면
OS가 상태를 알게 됩니다.

> Caps Lock으로 전환한다면 macOS의 탭/홀드 판별 때문에 지연이 붙습니다.
> `hidutil`로 Caps Lock을 F19로 돌리고 "입력 메뉴에서 다음 소스 선택"에 바인딩하면
> 없앨 수 있습니다. ⌃Space 같은 조합키는 해당 없습니다.

### 앱별 한/영 고정

트레이 메뉴 → **‘앱이름’에서 항상** → `지정 안 함` / `한글` / `영문`

지정한 앱이 앞으로 나오면 입력 소스를 그 언어로 맞춰줍니다. **지정하지 않은 앱은
건드리지 않습니다** — 모든 앱을 자동으로 기억하면 앱을 바꿀 때마다 언어가 바뀌어
예측이 어려워지므로, 명시적으로 지정한 앱만 다르게 동작합니다.

| 지정값 | 현재 입력 소스 | 동작 |
|---|---|---|
| 한글 | ABC | NavilIME로 전환 |
| 한글 | NavilIME | 그대로 |
| 영문 | ABC | **그대로** (이미 영문이라 건드릴 이유가 없음) |
| 영문 | NavilIME | ABC로 전환 |

감지 신호는 두 가지를 함께 씁니다:

- **`activateServer`** — IMK가 새 클라이언트로 입력기를 켤 때. Raycast·Spotlight 같은
  런처는 "비활성화 패널"로 떠서 **최전면 앱이 바뀌지 않으므로**, 이 경로로만 잡힙니다.
- **`NSWorkspace` 앱 활성화 알림** — 입력 소스가 ABC여서 입력기가 비활성일 때는
  `activateServer`가 오지 않으므로 이쪽이 필요합니다.

적용은 **앱이 바뀔 때만** 합니다. `activateServer`는 같은 앱 안에서도 자주 불리는데,
매번 적용하면 ⌘Space로 직접 바꾼 것을 즉시 되돌려버려 그 앱에서 반대 언어를 아예 못 쓰게
됩니다. 이 가드가 전환 루프도 같이 막습니다.
...
<details>
<summary>원본 README (원작자 Manwoo Yi)</summary>
